...
-   Interaction between Treatment and Time - if interaction of treatment and time are not needed, then repeated measures is not required. Therefore the effect of time on treatments cannot be studied.
...
```
Just one out of six did not contain zero in it. 
...
why numerical in "form" argument? because we want the means of the dap 
...
p = no of datapoints to go back in, q is the moving average 
...
correlation and semivariogram move in opposite directions. Range is the value(on x axis) after which the correlation/ semivariogram plateaus out. 
## empirical autocorrelation function
...
range interval does not include 0, so it is significant, but lower variance as compared to exponential. 
...
[1] is for the first component of ranef.
...
interaction between k rate and n rate, and fdap. Since n rate and k rate are part of the interaction, they dont need to be compared even if they were significant. 
